...
namespace Listeners.Forms.Roles.Secretary.Directories
{
    public partial class Qualification : Form
    {
        ...
        {
            dataGridViewEmployments.DataSource = sqlClient.GetDataTable("SELECT id AS 'Номер', name AS 'Квалификация' from qualification " + search + sort);
        ...
        private void buttonCreate_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Заполните поле квалификации", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (EmploymentIsExist() != string.Empty)
            {
                MessageBox.Show("Данный вид квалификации уже существует", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            bool isExecute = sqlClient.ExecuteRequest("INSERT INTO qualification (name, modifiedBy)" +
                " VALUES ('" + textBoxEmployment.Text.Trim() + "', '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')");

            if (!isExecute)
            {
                MessageBox.Show("Не удалось добавить вид квалификации", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            MessageBox.Show("Успешно добавлен новый вид квалификации", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ClearFields();
            ResetCondition();
            sort = " order by qualification.id desc ";
            LoadData();
        }
        ...
        {
            string value = sqlClient.GetValue("SELECT id from qualification where name='" + textBoxEmployment.Text + "'");
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            if (textBoxEmployment.Text.Trim() == string.Empty)
            {
                MessageBox.Show("Заполните поле вида квалификации", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            bool isExecute = sqlClient.ExecuteRequest("UPDATE qualification set name=" +
                "'" + textBoxEmployment.Text.Trim() + "', modifiedBy='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where id='" +
                primaryKey + "'");

            if (!isExecute)
            {
                MessageBox.Show("Не удалось изменить вид квалификации", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            MessageBox.Show("Успешно изменен вид квалификации", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ClearFields();
            ResetCondition();
        ...
        private void FillAfterClick()
        {
            ...
            textBoxEmployment.Text = dataGridViewEmployments.SelectedRows[0].Cells["Квалификация"].Value.ToString();
        }
        ...
        {
            return sqlClient.GetValue("SELECT COUNT(*) from course where qualification='" + primaryKey + "'") == "0";
        }
        ...
        private void удалениеToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            bool isExecute = sqlClient.ExecuteRequest("DELETE FROM qualification where id='" + primaryKey + "'");

            if (!isExecute)
            {
                MessageBox.Show("Не удалось удалить вид квалификации", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            MessageBox.Show("Успешно удален вид квалификации", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ClearFields();
            LoadData();
            buttonUpdate.Enabled = false;
        }
        ...
        private void buttonImport_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialogCsv.ShowDialog() == DialogResult.OK)
            {
                ...
                if (!csvHelper.ImportQualificationData(selectedFileName))
                {
        ...
        private void buttonExport_Click(object sender, EventArgs e)
        {
            if (folderBrowserDialogCsv.ShowDialog() == DialogResult.OK)
            {
                ...
                if (!csvHelper.ExportData(selectedDirectory, "qualification.csv"))
                {
                    MessageBox.Show("Не удалось экспортировать данные", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                MessageBox.Show("Данные успешно экспортированы в: " + Path.Combine(selectedDirectory, "qualification.csv"), "Внимание",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
    }
